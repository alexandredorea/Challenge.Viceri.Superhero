...
public sealed class UpdateHeroCommandValidatorDetalhado : AbstractValidator<UpdateHeroCommand>
{
    ...
    private void ConfigureSynchronousValidation()
    {
        ...
        RuleFor(x => x.Weight)
            ...
            .LessThan(MaxWeight).WithMessage("O peso não pode ser maior que {ComparisonValue} kg.");
    ...
    {
        RuleFor(x => x.Id)
            .MustAsync(HeroExistsAsync).WithMessage(x => $"Herói com Id '{x.Id}' não foi encontrado.");

        RuleFor(x => x)
            .MustAsync(CodenameIsUniqueAsync).WithMessage(x => $"Já existe outro herói com o nome '{x.Codename}'.");
